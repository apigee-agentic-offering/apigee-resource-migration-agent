    ...
    for dev_file in json_files:
        ...
        try:
            ...
            if success:
                ...
                STATS["failures"] += 1
        except Exception as e: log(f"Error: {e}", "ERROR", 1)
...
def main():
    ...
    if DEV_REGISTRY:
        ...
        if os.path.exists(registry_file):
            with open(registry_file, 'r') as f: existing = json.load(f)
        unique = {item["email"]: item for item in (existing + DEV_REGISTRY)}
        with open(registry_file, 'w') as f: json.dump(list(unique.values()), f, indent=2)
        log("Registry JSON successfully updated.", "SUCCESS")
